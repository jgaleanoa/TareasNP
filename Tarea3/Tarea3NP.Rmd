...
perder peso son identicos, por lo que se plantea el conjunto de hipótesis
...
respectivamente. Para este caso $n_i = 12 \ \forall_i$, luego $N = 3 \times 12 = 36$.
...
Dado el rechazo de $H_0$ se pueden realizar comparaciones multiples para 
...
mult_comp <- data.frame(Medias = medias, Diferencias = diffs,
                        Desigualdad = desigualdad, Valor_critico = valores_criticos)
...
$$Rc=\{T_1/T_1>Z_{0.975} \vee T<Z_{0.025}\}$$
```{r}
a <- (n*(N+1)*(2*N+1))/6
...
Como $T_{obs}\in Rc$ y $p_{valor}<0.05$ se rechaza $H_0$, es decir hay evidencia muestral para suguerir que existe diferencia entre la varianza de la tasa de flujo actual y la taza de flujo pasada del arroyo.
...
Sea $X$ el puntaje del esposo en bolos y $Y$ el puntaje de la espasa en bolos. Los rangos asignados para cada variable son:
...
Como el $|\rho|\notin Rc$ y el $p_{valor}>0.05$ se rechaza la hipotesis nula, es deicir, no hay evidencia muestral para suguerir que los $X_i$ y los $Y_i$ no son mutuamente independientes.
...
H_1: \text{Los pares de observaciones tienen a ser concordantes o discordantes}
\end{cases}
$$
...
Como $\tau\in Rc$ y el $p_{valor}$ esta muy cercano  a la frontera de significancia $\alpha=0.05$ se rechaza $H_0$, es decir, hay evidencia muestras para suguerir que los pares de observaciones tienden a ser concordantes o discordantes. 
