...
```{r RR_EJ1, echo=F}
t_ej1 <- qbinom(0.05, 25, 0.3, lower.tail = F)
```
...
por lo tanto se toma $t = 11$. Como se está en un prueba de dos colas la región críticia se define como $R_c = \lbrace T|T > t \rbrace$, en este caso se tiene $T = 3 \ngtr 11$, es decir, $T$ no cae en la región crítica por lo que a un nivel de significancia de aproximadamente el 0.0442 < 0.05 no hay evidencia para rechazar $H_0$ y se llega a la misma conclusión que se obtuvo haciendo uso del valor-p.
