...
Para probar la hipótesis anterior se toma $\alpha=0.05$, se toman como estadísticos de prueba $T_1$ el número
de observaciones menores o iguales a $x^*$,$T_2$ el número
de observaciones menores a $x^*$ y se crea una región critica $Rc$ tal que:
...
t2 <- ifelse(pbinom(q=qbinom(1-alpha/2,size=n,prob = p_star),size=n,prob=p_star)>1-alpha/2,
             qbinom(1-alpha/2,size=n,prob = p_star)-1,
...
Con los datos de los estudiantes de grado 10 se encuentra que $t_1=5$ y $t_2=13$, así la región critica queda como:

$$
Rc=\{(T_1,T_2)/T_1\leq 5 \vee T_2>13\}
$$

Luego los estadísticos calculados son: $T_1=6$ Y $T_2=4$

Como ni $T_1$ ni $T_2$  caen en la región critica no se rechaza $H_0$, por lo tanto no hay evidencia muestral suficiente para sugerir que la mediana de los estudiantes de grado 10 no sea 103.
...
Como $T_{2obs}$ cae en $Rc$ se rechaza $H_0$, es decir, hay evidencia muestral para sugerir que despues del boletin informativo hay un mayor cambio de las personas que se oponen.